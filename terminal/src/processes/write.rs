...
pub async fn write_chunk(terminal_id: &TerminalId, data: &[u8]) -> Result<(), WriteChunkError> {
    trace!("Writing {}", String::from_utf8_lossy(data).escape_default());
    let processes = get_processes();
    let entry = {
        let Some(entry) = processes.get(terminal_id) else {
            return Err(WriteChunkError::TerminalNotFound {
    ...
    return input.write_all(data).await.map_err(WriteChunkError::Write);
}
...
impl IsHttpError for WriteChunkError {
